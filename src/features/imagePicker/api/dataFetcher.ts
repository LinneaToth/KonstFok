...
export const getArtworkData = async (
  ...
  //If id is a single number, just keep as is. If it is an array, reformat it to a string that can be used to call the api endpoint below
  const searchId = Array.isArray(id) ? id.join(",") : id;

  const artworkDataFetcher = async () => {
    try {
      const imageRes = await fetch(
        `https://api.artic.edu/api/v1/artworks?ids=${searchId}&fields=id,title,image_id,is_public_domain,iiif_url,website_url,description,artwork_type_title`,
      );
      if (!imageRes.ok)
        throw new Error(
  ...
  if (typeof id === "number") return await artworkDataFetcher();
